...
class RealBorn(TN):
    ...
    def _computenorm(self):
        ...
        """
        w2 = cp.reshape(self.w,(self.n_features,self.d,self.D,self.D))
        tmp = cp.tensordot(w2[0,:,0,:],w2[0,:,0,:],axes=([0],[0])).reshape(self.D*self.D) #First tensor
        for i in range(1,self.n_features-1):
            tmp = cp.dot(tmp,cp.tensordot(w2[i,:,:,:],w2[i,:,:,:],
                    axes=([0],[0])).transpose((0,2,1,3)).reshape(self.D*self.D,self.D*self.D)) #MPS contraction  
        norm = cp.inner(tmp,cp.tensordot(w2[self.n_features-1,:,:,0],
    ...
    def _derivative(self, x):
        ...
        mpscontracted=cp.inner(tmp[self.n_features-2,:],
        ...
        for i in range(1,self.n_features-1):
                derivative[i,x[i],:,:]=2*cp.outer(tmp[i-1,:],tmp2[i+1,:])*mpscontracted
    ...
    def _derivativenorm(self):
        ...
        tmp=cp.zeros((self.n_features,self.D*self.D))
        tmp2=cp.zeros((self.n_features,self.D*self.D))
        tmp[0,:]=cp.tensordot(w2[0,:,0,:],w2[0,:,0,:],axes=([0],[0])).reshape(self.D*self.D)
        for i in range(1,self.n_features-1):
            tmp[i,:]=cp.dot(tmp[i-1,:],cp.tensordot(w2[i,:,:,:],w2[i,:,:,:],
                axes=([0],[0])).transpose((0,2,1,3)).reshape(self.D*self.D,self.D*self.D))
        tmp[self.n_features-1,:]=cp.inner(tmp[self.n_features-2,:],
            cp.tensordot(w2[self.n_features-1,:,:,0],w2[self.n_features-1,:,:,0],axes=([0],[0])).reshape(self.D*self.D))
        
        tmp2[self.n_features-1,:]=cp.tensordot(w2[self.n_features-1,:,:,0],
            w2[self.n_features-1,:,:,0],axes=([0],[0])).reshape(self.D*self.D)
        for i in range(self.n_features-2,-1,-1):
            tmp2[i,:]=cp.dot(cp.tensordot(w2[i,:,:,:],
                w2[i,:,:,:],axes=([0],[0])).transpose((0,2,1,3)).reshape(self.D*self.D,self.D*self.D),tmp2[i+1,:])
        tmp2[0,:]=cp.inner(cp.tensordot(w2[0,:,0,:],w2[0,:,0,:],
                axes=([0],[0])).reshape(self.D*self.D),tmp2[1,:])
        

        for j in range(self.d):
            derivative[0,j,0,:]=2*cp.dot(tmp2[1,:].reshape(self.D,self.D),
                                                        w2[0,j,0,:])
            derivative[self.n_features-1,j,:,0]=2*cp.dot(tmp[self.n_features-2,:].reshape(self.D,self.D),
        ...
        for i in range(1,self.n_features-1):
            ...
            for j in range(self.d):
                temp3=cp.dot(cp.dot(temp1,w2[i,j,:,:]),temp2.transpose())
